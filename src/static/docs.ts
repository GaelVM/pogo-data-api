const endpoints = [
  ...
  ['Recursos', 'Sprites', 'v1/sprites.json', 'URLs de sprites y arte oficial servidos por PokéAPI.'],
] as const
...
function renderDex(){const q=document.getElementById('pokemonSearch').value.toLowerCase().trim();const all=dex.filter(p=>dexMatches(p,q));const visible=all.slice(0,dexLimit);document.getElementById('dexGrid').innerHTML=visible.map(p=>'<a class="poke" href="v1/pokemon/'+p.id+'.json" target="_blank" rel="noreferrer"><img loading="lazy" src="'+safe(p.sprites.default)+'" alt=""><small>#'+String(p.id).padStart(4,'0')+'</small><b>'+safe(p.name)+'</b><div class="types">'+p.types.map(t=>'<span class="type">'+safe(t.name)+'</span>').join('')+'</div></a>').join('')||'<p class="muted">Sin resultados.</p>';document.getElementById('dexMore').classList.toggle('hidden',visible.length>=all.length)}
